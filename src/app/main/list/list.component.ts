...
import {interval, Observable, Subscription, OperatorFunction, Subject, of} from "rxjs";
...
export class ListComponent implements OnInit {
    ...
    constructor(
        ...
        // websocket
        try {
            // const mapped$ = listService.messages.pipe(
            //     debounceTime(1),
            ...
            //             debugger;
            //             console.log("term" + x)
            //             console.log("wc-user2: " + x)
            //             this.getAllOfList(this.page)
            //             return of(x);
            //         }
            //     )
            // );
            // listService.messages.pipe(
            //     debounceTime(1),
            //     distinctUntilChanged(),
            //     map(term => {
            //             console.log("term" + term)
            //             console.log("wc-user2: " + term)
            //             this.getAllOfList(this.page)
            //         }
            //     )
            // )
    ...
    search() {
        ...
        //
        // try {
        //     this.getMessage2(this.listService.messages)
    ...
    getAllOfList(page) {
        ...
        this.listService.getAllOfList(this.formSearch.value.search, this.pagedData.currentPage + 1, this.pagedData.size
            ...
            if (res.isSuccess === true) {
                ...
                this.pagedData.totalPages = res.data.totalItems / this.itemOnPage
                if (res.data.totalItems % this.itemOnPage > 0) {
                    this.pagedData.totalPages += 1
                }
